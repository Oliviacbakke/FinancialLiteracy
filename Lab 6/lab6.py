...
class BayesClassifier:
    ...
    def train(self, training_data: str):
        ...
        for file in files:
            ...
            if NEGATIVE_FILE_PREFIX in file:
                self.neg_freqs = self.update_dict(data, self.neg_freqs)
            else:
                self.pos_freqs = self.update_dict(data, self.pos_freqs)
    ...
    def classify(self, text: str) -> str:
        ...
        for word in words:
            pos_score += math.log(
                (self.pos_freqs.get(word, 0) + 1) / total_pos
            )
            neg_score += math.log(
                (self.neg_freqs.get(word, 0) + 1) / total_neg
            )
