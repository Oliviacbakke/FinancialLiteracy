...
# TODO: Define the constants as we did in lab 1
# You will need constants for host id, host name, room type,
#    price, min. number of nights, and license
# They can start with IDX_ or INDEX_, whatever you prefer
# e.g. IDX_HOST_ID or INDEX_HOST_ID
...
def get_host_name_by_id(data: list[list[str]], host_id: str) -> str:
    ...
    # TODO: Implement me
    for airbnb in data:
        if host_id == airbnb[INDEX_HOST_ID]:
            return airbnb[INDEX_HOST_NAME]
...
def count_listings_by_type(data: list[list[str]]) -> dict[str, int]:
    ...
    dict = {}
    for airbnb in data:
        if airbnb[INDEX_ROOM_TYPE] == '':
            continue
        if airbnb[INDEX_ROOM_TYPE] in dict.keys():
            dict[airbnb[INDEX_ROOM_TYPE]] += 1
        else:
            dict[airbnb[INDEX_ROOM_TYPE]] = 1
    return dict
...
def get_license_status(data: list[list[str]]) -> dict[str, int]:
    ...
    # TODO: Implement me!
    dict = {"unlicensed": 0, "pending": 0, "exempt": 0, "licensed": 0}
    for airbnb in data:
        lisence = airbnb[INDEX_LICENSE]
        if lisence == '':
            dict["unlicensed"] += 1
        elif 'pending' in lisence.lower():
            dict["pending"] += 1
        elif '32+' in lisence or '32-' in lisence or '32 +' in lisence or '32 -' in lisence:
            dict["exempt"] += 1
        else:
            dict["licensed"] += 1
    return dict
...
def count_multi_listings(data: list[list[str]]) -> int:
    ...
    duplicate = []
    for airbnb in data:
        if airbnb[INDEX_HOST_ID] in seen:
            if airbnb[INDEX_HOST_ID] in duplicate:
                count = count + 1
            else:
                count = count + 2
                duplicate.append(airbnb[INDEX_HOST_ID])
        seen.append(airbnb[INDEX_HOST_ID])
    return count
...
def count_listings_by_host_count(data: list[list[str]]) -> list[int]:
    ...
    for count in dict.values():
        if count >= 10:
            listing_counts[9] += count
        else:
            listing_counts[count - 1] += count
...
def get_prices(data: list[list[str]], room_type: str = "") -> list[float]:
    ...
    prices = []
    for airbnb in data:
        room = airbnb[INDEX_ROOM_TYPE]
        price = airbnb[INDEX_PRICE]
        if price == '':
            continue
        if room_type != '' and room != room_type:
            continue
...
def listings_per_host_with_type(data: list[list[str]], room_type: str = "") -> dict[str, int]:
    ...
    * If room type is non-empty (e.g. "Entire home/apt"),
    ...
    for airbnb in data:
        room = airbnb[INDEX_ROOM_TYPE]
        id = airbnb[INDEX_HOST_ID]
        if room_type == '' or room_type == room:
            dict[id] += 1
