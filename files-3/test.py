...
def test_read_data():
    ...
    """
    # Note that you do not need to add test cases to this function
    # test cases for data reading from a file are quite annoying to write
    #   and, outside of the format of the data, they don't give much information
    # these tests should be sufficient for this assignment

    fn_name = 'read_data'

    inp = TEST_PATH_1
    expected = [
        ['0', 'n/a', '172632', 'Sarah', 'n/a', 'n/a', '0', '0',
            'Entire home/apt', '5', '0', '0', '0', '0', '0', 'n/a', 'n/a', ''],
        ['0', 'n/a', '329038', 'Marvin', 'n/a', 'n/a', '0', '0',
            'Private room', '7', '200', '0', '0', '0', '0', 'n/a', 'n/a', '32-']
    ]
    ...
    inp = TEST_PATH_2
    expected = [
        ['0', 'n/a', '182736', 'Mittens', 'n/a', 'n/a', '0', '0', 'Entire home/apt',
            '1215', '29', '0', '0', '0', '0', 'n/a', 'n/a', "it's pending"],
        ['0', 'n/a', '182736', 'Mittens', 'n/a', 'n/a', '0', '0', 'Private room',
            '172', '30', '0', '0', '0', '0', 'n/a', 'n/a', 'R1723617236'],
        ['0', 'n/a', '182736', 'Mittens', 'n/a', 'n/a', '0', '0', 'Entire home/apt',
            '12798', '235', '0', '0', '0', '0', 'n/a', 'n/a', '2732+'],
        ['0', 'n/a', '127377', 'Caeser', 'n/a', 'n/a', '0', '0', 'Entire home/apt',
            '2374', '5', '0', '0', '0', '0', 'n/a', 'n/a', 'R12873132-'],
        ['0', 'n/a', '127377', 'Caeser', 'n/a', 'n/a', '0', '0',
            'Private room', '2374', '100', '0', '0', '0', '0', 'n/a', 'n/a', ''],
        ['0', 'n/a', '126357', 'Gigi', 'n/a', 'n/a', '0', '0', 'Entire home/apt',
            '37263', '3000', '0', '0', '0', '0', 'n/a', 'n/a', 'pending']
    ]
    result = report.read_data(inp)
    error_message = f'expected {expected} for {fn_name}({inp}), got {result}'
    assert expected == result, error_message
...
def test_get_host_name_by_id():
    ...
    # TODO: Add at least 2 new tests

    file_path = TEST_PATH_3
    inp = report.read_data(file_path)
    expected = ''
    id_to_test = ''
    result = report.get_host_name_by_id(inp, '')
    error_message = f'expected {expected} for {fn_name}(...{file_path}, {id_to_test}), got {result}'
    assert expected == result, error_message
    ...
    expected = 'Name not found'
    id_to_test = '102837'
    result = report.get_host_name_by_id(inp, '102837')
    error_message = f'expected {expected} for {fn_name}(...{file_path}, {id_to_test}), got {result}'
    assert expected == result, error_message
...
def test_get_license_status():
    ...
    file_path = TEST_PATH_3
    inp = report.read_data(file_path)
    expected = {'unlicensed': 1, 'pending': 0, 'exempt': 0, 'licensed': 0}
    result = report.get_license_status(inp)
    error_message = f'expected {expected} for {fn_name}(...{file_path}), got {result}'
    assert expected == result, error_message
    ...
    file_path = TEST_PATH_4
    inp = report.read_data(file_path)
    expected = {'unlicensed': 0, 'pending': 0, 'exempt': 0, 'licensed': 0}
    result = report.get_license_status(inp)
    error_message = f'expected {expected} for {fn_name}(...{file_path}), got {result}'
    assert expected == result, error_message
