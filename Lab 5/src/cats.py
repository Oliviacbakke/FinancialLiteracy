...
class Cat(Actor):
    ...
    def move(self):
        ...
        from src.contraptions import Ball
        ...
        elif isinstance(self.tile().exit().actor(), Ball):
            self.distract(1)
            self.tile().exit().actor().interact()
            if self.tile().exit().is_empty():
                self.teleport(self.tile().exit())

        elif isinstance(self.tile().exit().actor(), SpaceHeater):
            self.distract(1)
...
class Calico(Cat):
    ...
    def distract(self, amount: int):
        assert isinstance(amount, int) and amount >= 0
        self._attention -= amount
        if self.tile().above() is not None and self.tile().above().is_empty() == True:
            self.teleport(self.tile()._above)
        if self._attention <= 0:
            self._attention = 0
            self._rest_time = self._needed_rest_time
            self.remove_from_board()
...
class Kitten(Cat):
    ...
    def move(self):
        ...
        elif isinstance(self.tile().exit().exit().actor(), SpaceHeater):
            self.distract(1)
            self.tile().exit().exit().actor().interact()
